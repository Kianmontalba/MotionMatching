...
MMDatabaseEditor::MMDatabaseEditor() {
	...
	_pelvis_override = memnew(LineEdit);
	_pelvis_override->set_placeholder("Leave empty to auto-detect");
	_add_compact_box(_advanced_section, "Pelvis bone (override)", _pelvis_override);
	...
	_clip_table->set_column_expand(0, false);
	_clip_table->set_column_expand(1, true);
	_clip_table->set_column_expand(2, false);
	_clip_table->set_column_expand(3, false);
	_clip_table->set_column_expand(4, false);
	_clip_table->set_column_custom_minimum_width(0, 90);
	_clip_table->set_column_custom_minimum_width(2, 90);
	_clip_table->set_column_custom_minimum_width(3, 50);
	_clip_table->set_column_custom_minimum_width(4, 60);
...
void MMDatabaseEditor::_ready() {
	...
	_pelvis_override->connect("text_changed", Callable(this, "_on_pelvis_override_changed"));
	...
	_clip_table->connect("item_edited", Callable(this, "_on_clip_edited"));
...
void MMDatabaseEditor::_on_resource_picked(const Ref<Resource> &p_resource) {
	...
	_pelvis_override->set_text(_mm_resource->get_pelvis_override());
...
void MMDatabaseEditor::_on_pelvis_override_changed(const String &p_text) {
	if (_mm_resource.is_null()) {
		return;
	}
	// Same persist-immediately pattern as _on_skeleton_path_changed().
	_mm_resource->set_pelvis_override(p_text);
	if (_mm_resource->get_path().is_empty()) {
		return;
	}
	const Error error = ResourceSaver::get_singleton()->save(_mm_resource, _mm_resource->get_path());
	if (error != OK) {
		_log_line(vformat("Could not save the pelvis override back to %s (error %d).",
							_mm_resource->get_path(), (int)error),
				Color(1, 0.5f, 0.4f));
	}
}
...
void MMDatabaseEditor::_populate_table() {
	...
	for (int i = 0; i < names.size(); i++) {
		...
		// Animation names like "Rifle/M_Neutral_AO_Stand_X+45_Y0_Rifle" carry a
		// group prefix before the first "/" -- the animation set or sub-folder
		// it was imported under. Splitting it into its own column means that
		// prefix does not have to be repeated (and re-read) in every single
		// clip name.
		const int slash = names[i].find("/");
		const String group = slash >= 0 ? names[i].substr(0, slash) : String("-");
		const String clip_name = slash >= 0 ? names[i].substr(slash + 1) : names[i];

		TreeItem *item = _clip_table->create_item(root);
		item->set_text(0, group);
		item->set_text(1, clip_name);
		...
		// The full, unsplit name is what every lookup elsewhere (_clip_settings,
		// build_database's per-clip settings dictionary) is keyed by -- kept in
		// metadata so edits below can still resolve the right entry even though
		// column 1 only shows the part after the group prefix.
...
void MMDatabaseEditor::_on_build_pressed() {
	...
	const String pelvis = _pelvis_override->get_text().strip_edges();
	...
	if (!pelvis.is_empty() || !left_foot.is_empty() || !right_foot.is_empty()) {
		// Only these three roles are ever set by hand here -- everything else
		...
		// so this survives rebuilds instead of being overwritten. Setting all
		// three also lets the build proceed even if auto-detect's heuristics
		// fail on this rig's shape -- see MMFeatureExtractor::_prepare().
		Ref<MMSkeletonProfile> profile;
		profile.instantiate();
		if (!pelvis.is_empty()) {
			profile->set_bone_name(MM_BONE_PELVIS, pelvis);
		}
...
void MMDatabaseEditor::_on_clip_edited() {
	...
	// Column 1 only shows the part after the group prefix now (see
	// _populate_table()) -- the full name every lookup is actually keyed by
	// lives in this item's metadata instead.
	const String full_name = item->get_metadata(0);
	Dictionary settings = _clip_settings.get(full_name, Dictionary());
	settings["tags"] = item->get_text(3).to_int();
	const int category = MMFeatureExtractor::guess_category_from_tags(settings["tags"]);
	settings["category"] = category;
	_clip_settings[full_name] = settings;

	// Applied immediately, same signal handler -- no separate "commit"/"apply"
	// step -- so the Category column reflects a typed tag value the instant
...
void MMDatabaseEditor::_bind_methods() {
	...
	ClassDB::bind_method(D_METHOD("_on_pelvis_override_changed", "text"), &MMDatabaseEditor::_on_pelvis_override_changed);
